  ...
  return createPortal(
    <div className="fixed inset-0 z-[9998] flex min-h-screen w-screen justify-center p-4 pt-[20vh] tablet:pt-[30vh]">
      ...
      <form ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="product-modal-title" onSubmit={add} className="relative z-[9999] flex h-fit min-h-[220px] w-[80vw] max-w-lg flex-col gap-4 rounded-lg border border-darkcoral bg-white p-4 pt-8 shadow-xl outline-none">
        ...
        {selectedProduct && <div className="flex flex-col gap-2 items-center"><label htmlFor="product-quantity" className="text-lg">Ilość: ({selectedProduct.packagingMethod})</label><div className="flex gap-2 items-center"><input id="product-quantity" type="number" min="0.1" step="0.1" value={quantity} onChange={(event) => setQuantity(event.target.value)} className="w-[100px] border border-[#CCCCCC] p-1 text-lg" /><button type="button" aria-label="Zwiększ ilość" onClick={() => setQuantity((current) => Number((Number(current) + 0.1).toFixed(1)))}><CirclePlus className="w-[2rem] h-auto" /></button><button type="button" aria-label="Zmniejsz ilość" onClick={() => setQuantity((current) => Math.max(0.1, Number((Number(current) - 0.1).toFixed(1))))}><CircleMinus className="w-[2rem] h-auto" /></button></div></div>}
