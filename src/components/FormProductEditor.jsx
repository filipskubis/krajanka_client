...
export default function FormProductEditor({ product, claimedItemIds = [], onChange, onRemove }) {
  ...
  const addWeight = () => {
    ...
    if (!/^\d+(?:\.\d{1,2})?$/.test(normalized) || Number(normalized) <= 0) { setError("Podaj dodatnią wagę z maksymalnie dwoma miejscami po przecinku."); return; }
  ...
  return <section className="flex flex-col gap-3 rounded-md border p-3">
    ...
    {!weighted ? <div className="flex flex-col gap-2 items-start"><label htmlFor={`quantity-${product.id}`}>Ilość:</label><div className="flex gap-2 items-center"><input id={`quantity-${product.id}`} type="number" min="1" step="0.01" value={product.quantity} onChange={(event) => onChange({ ...product, quantity: event.target.value })} className="w-[80px] border-[1px] border-[#CCCCCC] p-1 text-lg" /><button type="button" aria-label={`Dodaj ${product.name}`} onClick={() => onChange({ ...product, quantity: Number(product.quantity) + 1 })} className="grid h-9 w-9 place-items-center rounded focus-visible:ring-2 focus-visible:ring-coral"><CirclePlus /></button><button type="button" aria-label={`Odejmij ${product.name}`} onClick={() => Number(product.quantity) > 1 ? onChange({ ...product, quantity: Number(product.quantity) - 1 }) : onRemove()} className="grid h-9 w-9 place-items-center rounded focus-visible:ring-2 focus-visible:ring-coral"><CircleMinus /></button></div></div> : <>
