...
export default function FormProductEditor({ product, claimedItemIds = [], onChange, onRemove }) {
  ...
  const quantityStep = getQuantityStep(product.packagingMethod);
  ...
  return <section className="flex flex-col gap-3 rounded-md border p-3">
    ...
    {!weighted ? <div className="flex flex-col gap-2 items-start"><label htmlFor={`quantity-${product.id}`}>Ilość: ({product.packagingMethod})</label><div className="flex gap-2 items-center"><input id={`quantity-${product.id}`} type="number" min={quantityStep} step={quantityStep} value={product.quantity} onChange={(event) => { const nextQuantity = Number(event.target.value); if (Number.isFinite(nextQuantity) && nextQuantity >= quantityStep) onChange({ ...product, quantity: normalizeQuantity(nextQuantity, product.packagingMethod) }); }} className="w-[80px] border-[1px] border-[#CCCCCC] p-1 text-lg" /><button type="button" aria-label={`Dodaj ${product.name}`} onClick={() => onChange({ ...product, quantity: normalizeQuantity(Number(product.quantity) + quantityStep, product.packagingMethod) })} className="grid h-9 w-9 place-items-center rounded focus-visible:ring-2 focus-visible:ring-coral"><CirclePlus /></button><button type="button" aria-label={`Odejmij ${product.name}`} onClick={() => Number(product.quantity) > quantityStep ? onChange({ ...product, quantity: normalizeQuantity(Number(product.quantity) - quantityStep, product.packagingMethod) }) : onRemove()} className="grid h-9 w-9 place-items-center rounded focus-visible:ring-2 focus-visible:ring-coral"><CircleMinus /></button></div></div> : <>
